#!/bin/bash
# CyberPot Startup Script for GCP Compute Instance
# This script runs during instance boot to configure CyberPot
...
log "Starting CyberPot bootstrap process on GCP..."
...
# Add current user to docker group (assuming ubuntu user)
usermod -aG docker ubuntu 2>/dev/null || usermod -aG docker $(whoami)
...
# Mount data disk if available
log "Checking for additional data disk..."
if lsblk | grep -q sdb && ! mount | grep -q sdb1; then
    log "Found additional disk, formatting and mounting..."
    parted /dev/sdb --script mklabel gpt mkpart primary ext4 0% 100%
    mkfs.ext4 /dev/sdb1
    mkdir -p /home/ubuntu/cyberpot/data
    mount /dev/sdb1 /home/ubuntu/cyberpot/data
    echo "/dev/sdb1 /home/ubuntu/cyberpot/data ext4 defaults,nofail 0 2" >> /etc/fstab
...
# Configure Google Cloud Ops Agent for enhanced monitoring
  ...
  service:
    pipelines:
      default_pipeline:
        ...
        processors: [metricstransform/hostmetrics]
EOF
...
# Create daily maintenance cron job (as per CyberPot recommendations)
...
# Create CyberPot management scripts
...
# Create backup script with GCP integration
...
if command -v gsutil &> /dev/null; then
    ...
    gsutil cp $BACKUP_DIR/cyberpot_backup_$DATE.tar.gz gs://cyberpot-backups-$(curl -s "http://metadata.google.internal/computeMetadata/v1/project/project-id" -H "Metadata-Flavor: Google")/
fi
...
# CyberPot GCP Compute Instance

## Access Information

**Web UI:** https://${domain_name}:64297
**SSH Access:** gcloud compute ssh --zone=$(curl -s "http://metadata.google.internal/computeMetadata/v1/instance/zone" -H "Metadata-Flavor: Google" | cut -d/ -f4) $(curl -s "http://metadata.google.internal/computeMetadata/v1/instance/name" -H "Metadata-Flavor: Google") --tunnel-through-iap
...
- Backups are created daily and uploaded to Cloud Storage

## Security

- Firewall is configured with UFW
- Only necessary ports are open
- Google Cloud Ops Agent provides monitoring
- All disks are encrypted with Google managed keys
- VPC Flow Logs are enabled for network monitoring

## GCP Integration

- Cloud Monitoring alerts are configured
- Cloud Logging collects CyberPot logs
- Cloud Storage is used for backup storage
- Instance metadata provides system information
...
5. Check GCP logs: gcloud logging read "resource.type=gce_instance AND resource.labels.instance_id=$(curl -s "http://metadata.google.internal/computeMetadata/v1/instance/id" -H "Metadata-Flavor: Google")"
...
- View instance: https://console.cloud.google.com/compute/instancesDetail/zones/$(curl -s "http://metadata.google.internal/computeMetadata/v1/instance/zone" -H "Metadata-Flavor: Google" | cut -d/ -f4)/instances/$(curl -s "http://metadata.google.internal/computeMetadata/v1/instance/name" -H "Metadata-Flavor: Google")?project=$(curl -s "http://metadata.google.internal/computeMetadata/v1/project/project-id" -H "Metadata-Flavor: Google")
- View logs: https://console.cloud.google.com/logs/query;query=resource.type%3Dgce_instance%0Aresource.labels.instance_id%3D%22$(curl -s "http://metadata.google.internal/computeMetadata/v1/instance/id" -H "Metadata-Flavor: Google")%22?project=$(curl -s "http://metadata.google.internal/computeMetadata/v1/project/project-id" -H "Metadata-Flavor: Google")
EOF
